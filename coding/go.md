...
func main() {
	...
	for range 10 {
		wg.Add(1) // add 1 waitgroup
		go func() {
			res++
			wg.Done() // remove 1 waitgroup
		}()
	}
...
func main() {
	...
	// still requires a wait group but instead of waiting 1 by 1, we can immediately wait for all the goroutines
    wg.Add(3)
